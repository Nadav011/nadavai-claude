...
ALLOW_RE='^(DESIGN\.md|PRODUCT\.md|\.claude/rules/[^/]+\.md|\.claude/settings\.local\.json|\.claude/scheduled_tasks\.lock|docs/archive/|design/[^/]+/DESIGN\.md|docs/specs/|docs/ui-audit/|e2e/dod/|playwright\.dod\.config\.ts|\.claude/rules/ui\.md|\.claude/settings\.json|\.mcp\.json|\.omc/archive/|\.omc/design-runs/|\.github/workflows/ui\.yml|\.github/PULL_REQUEST_TEMPLATE\.md|lighthouserc\.json|package\.json|CLAUDE\.md|AGENTS\.md|STATUS\.md|BACKLOG\.md|.*/base\.css|.*styleguide.*|app/accessibility/|app/privacy/)'
verify=0; [ "${1:-}" = "--verify" ] && verify=1
names="$({ git ls-files -co --exclude-standard; git ls-files -oi --exclude-standard; } | sort -u | grep -viE '^(node_modules|\.next|dist|build|public|app|src|components|lib|styles|supabase|android|ios|functions|\.omc/(state|logs|handoffs|artifacts)|reports|test-results|playwright-report)/' | grep -iE "$NAME_RE" || true)"
texts="$(grep -rilE "$TEXT_RE" --include='*.md' --include='*.json' --include='*.mdc' --include='*.txt' --include='*.html' --exclude-dir=node_modules --exclude-dir=.next --exclude-dir=.git --exclude-dir=dist --exclude-dir=build --exclude-dir=app --exclude-dir=src --exclude-dir=components --exclude-dir=lib --exclude-dir=public --exclude-dir=supabase --exclude-dir=android --exclude-dir=ios --exclude-dir=functions --exclude-dir=state --exclude-dir=logs --exclude-dir=handoffs --exclude-dir=artifacts --exclude-dir=reports --exclude-dir=test-results --exclude-dir=playwright-report . 2>/dev/null | sed 's|^\./||' || true)"
