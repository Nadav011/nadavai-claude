...
- Components come from shadcn (`{PM_DLX} shadcn@latest add <name>`); do not hand-write one
  shadcn already has.
- A visible change is never merged on the agent's judgement: whenever the rendered result changes (colour, font, spacing, position, size, icon, motion, copy, a state), stop, show the before and after at 375 and 1440, and ask Nadav whether to keep, change or revert. Why: he is the one who judges how it looks, and a change nobody looked at is how an app drifts.
- No open debt and no open question: every finding ends as fixed, as a `- [ ]` item in `docs/ui-audit/PLAN.md`, or as a stated exception in DESIGN.md; every question is asked when it arises, with concrete options.
