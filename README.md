...
| Dependencies: oh-my-claudecode (OMC), i-have-adhd, context7, typescript-lsp, pyright-lsp, playwright, supabase, chrome-devtools-mcp, cloudflare, skill-creator; UI/UX and product layer: impeccable, intent, interfaces, ux-design, product-innovation, product-management, posthog | `plugins/nadavai/.claude-plugin/plugin.json` | installed and enabled automatically with the bundle |
...
| Repo-sync hooks (SessionStart status, PostToolUse edit reminder) and the SessionStart project check (re-init pending while `PROJECT-FACTS.md` exists, design layer pending while a UI project lacks `DESIGN.md` or `PRODUCT.md`) | `plugins/nadavai/hooks/`, `plugins/nadavai/scripts/` | the bundle |
...
| DoD gate template for design-init step 6 (Playwright + axe audit per route, viewport and colour scheme; writes `reports/dod/dod.json`) | `templates/dod/` | copied into a project by hand, see its README |
